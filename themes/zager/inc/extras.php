...
add_action('pre_get_posts', 'understrap_custom_query');
function understrap_custom_query($query) {
...
function get_fa_icon_as_svg( $icon ) {

	$class = $icon;
	$dir = '';

	// Icon passed an an object. We can work with this.
	if( is_object( $icon ) ) {
		$class = $icon->class;
	}

	// Icon passed an an HTML element. We can work with this.
	if( is_string( $icon ) && substr( $icon, 0, 2 ) === "<i" ) {
	...
	$type = $pieces[0];
	$icon = str_replace( 'fa-', '', $pieces[1] );

	if( 'fas' === $type ) $dir = 'solid';
	if( 'far' === $type ) $dir = 'regular';
	if( 'fab' === $type ) $dir = 'brands';
	...
	echo '<span class="fa-icon">';
	include( get_template_directory() . '/images/icons/'. $dir . '/' . $icon );
	echo '</span>';
