...
if ( ! function_exists( 'understrap_posted_on' ) ) {
	...
	function understrap_posted_on() {
		...
		$byline      = apply_filters(
			...
			sprintf(
				'<span class="byline"> <i class="far fa-user"></i> %1$s<span class="author vcard"> <a class="url fn n" href="%2$s">%3$s</a></span></span>',
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && understrap_categorized_blog() ) {
				...
				printf( '<span class="cat-links"><i class="far fa-folder-open"></i>' . esc_html__( '%s', 'understrap' ) . '</span>', $categories_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'understrap' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links"><i class="fas fa-tags"></i> ' . esc_html__( '%s', 'understrap' ) . '</span>', $tags_list ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped
			}
		}
		if ( ! is_single() && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
			echo '<span class="comments-link"><i class="far fa-comments"></i> ';
			comments_popup_link( esc_html__( 'Leave a comment', 'understrap' ), esc_html__( '1', 'understrap' ), esc_html__( '%', 'understrap' ) );
			echo '</span>';
		}
		edit_post_link(
			sprintf(
