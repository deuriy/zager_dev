...
define( 'WC_GATEWAY_AFFIRM_VERSION', '1.1.14' ); // WRCS: DEFINED_VERSION.
...
class WC_Affirm_Loader
{
    ...
    protected function __construct()
    {
        ...
        add_action('woocommerce_single_product_summary', array( $this, 'promoMessageAfterProductPrice' ), 15);
        add_action('woocommerce_after_add_to_cart_form', array( $this, 'promoMessageAfterAddToCart' ));
        ...
        add_action( 'woocommerce_admin_order_totals_after_total', array( $this, 'displayOrderFee' ) );

        //Affirm Inline Checkout
        add_action( 'woocommerce_checkout_after_order_review', array( $this, 'inlineCheckout' ) );
        add_action( 'wc_ajax_wc_affirm_inline_checkout', array( $this, 'ajaxInlineCheckout' ));
    ...
    /**
     * Conditionally render Affirm's monthly payment messaging to single product page after product price.
     *
     * @return void
     */
    public function promoMessageAfterProductPrice()
    {
        if ($this->getGateway()->productALA_options === 'after_product_price' ) {
            $this->woocommerceSingleProductSummary();
        }
    }

    /**
     * Conditionally render Affirm's monthly payment messaging to single product page after add to cart button.
     *
     * @return void
     */
    public function promoMessageAfterAddToCart()
    {
        if ($this->getGateway()->productALA_options === 'after_add_to_cart' ) {
            $this->woocommerceSingleProductSummary();
        }
    }
	...
	public function displayOrderFee($order_id) {
		...
		<tr>
			<td class="label affirm-fee">
                <?php echo wc_help_tip( 'This is the portion of the captured amount that represents the mertchant fee for the transaction.'); ?>
				Affirm Fee:
			</td>
			<td width="1%"></td>
			<td class="total">
	...
	public function inlineCheckout(){
        wp_register_script('affirm_inline_checkout', plugins_url('assets/js/affirm-inline-checkout.js', __FILE__), array( 'jquery' ));

        if($this->getGateway()->inline) {

            $cartItems = WC()->cart->get_cart();

            $is_affirm_selected = WC()->session->get('chosen_payment_method') == 'affirm';

            wp_localize_script(
                'affirm_inline_checkout',
                'affirmInlineCheckout',
                array(
                    'affirmInlineEnabled' => true,
                    'affirmSelected' => $is_affirm_selected ? true : false,
                    'affirmInlineEndpoint' => WC_AJAX::get_endpoint('wc_affirm_inline_checkout'),
                    'affirmInlineCheckoutObject' => $this->getInlineCheckoutObject()
                )
            );
        } else {
            wp_localize_script(
    ...
    public function getInlineCheckoutObject($nameArray = false){
        ...
        $checkoutObject = array(
            ...
            'items' => $this->formatInlineCheckout($cart->get_cart())
        );

        $checkoutObject['shipping'] = $this->formatAddress(
                $customer->get_shipping(),
            $nameArray ? $nameArray['email'] : '',
            $nameArray ? $nameArray['phone'] : '',
            $nameArray ? $nameArray['billingFirstName'] : '',
            $nameArray ? $nameArray['billingLastName'] : ''
        );
        $checkoutObject['billing'] = $this->formatAddress(
                $customer->get_shipping(),
            $nameArray ? $nameArray['email'] : '',
            $nameArray ? $nameArray['phone'] : '',
            $nameArray ? $nameArray['shippingFirstName'] : '',
            $nameArray ? $nameArray['shippingLastName'] : ''
        );
    ...
    private function formatInlineCheckout ($items){
    ...
    public function ajaxInlineCheckout(){

        $billFirsttName = $_POST['billing_first_name'] ? $_POST['billing_first_name'] : "";
        $billLasttName = $_POST['billing_last_name'] ? $_POST['billing_last_name'] : "";
        $shipFirstName = $_POST['shipping_first_name'] ? $_POST['shipping_first_name'] : $billFirsttName;
        $shipLastName = $_POST['shipping_last_name'] ? $_POST['shipping_last_name'] : $billLasttName;
        $phone = $_POST['billing_phone'] ? $_POST['billing_phone'] : "";
        $email = $_POST['billing_email'] ? $_POST['billing_email'] : "";

        $nameArray = array (
            'billingFirstName' => $billFirsttName,
            'billingLastName' => $billLasttName,
        ...
        wp_send_json($this->getInlineCheckoutObject($nameArray));

    }

    private function formatAddress($address , $email , $phone , $first, $last){
        $formattedAddress = false;
            $formattedAddress = array(
                ...
                'address' => array(
                    'line1'   => isset($address['address_1']) ? $address['address_1'] : '',
                    'line2'   => isset($address['address_2']) ? $address['address_2'] : '',
