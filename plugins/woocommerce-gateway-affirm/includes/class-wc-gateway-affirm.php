...
class WC_Gateway_Affirm extends WC_Payment_Gateway
{
    ...
    public function __construct()
    {
        ...
        $this->description    = $this->get_option('inline_messaging', 'yes') === 'yes' ?
        ...
        $this->productALA_options = $this->get_option('productALA_options');
        $this->cartALA = $this->get_option('cartALA', 'yes') === 'yes';
        $this->min     = $this->get_option('min');
        $this->max     = $this->get_option('max');
        $this->inline  = $this->get_option('inline_messaging', 'yes') === 'yes';
    ...
    public function captureCharge( $order )
    {
        ...
        $fee_amount = $result['fee_amount']; // Affirm provides amounts in cents

        // Save the fee amount on the order
        $this->updateOrderMeta($order_id, 'fee_amount', $fee_amount);
    ...
    public function initFormFields()
    {
        ...
        $this->form_fields = array(
            ...
            'title' => array(
                ...
                'default'     => __('Affirm Pay over time', 'woocommerce-gateway-affirm'),
                'desc_tip'    => true,
            ),
            'description' => array(
            ...
            'account_settings' => array(
                'title'       => __('Account Settings', 'woocommerce-gateway-affirm'),
                'type'        => 'select',
                'default'     => 'expand',
                'options'     => array(
            ...
            'inline_messaging' => array(
                'title'       => __('Inline Checkout Messaging', 'woocommerce-gateway-affirm'),
                'type'        => 'checkbox',
                'description' => __('Enable/Disable Inline checkout value props on the checkout page when Affirm is selected as a payment method', 'woocommerce-gateway-affirm'),
                'default'     => 'yes'
            ),
            'cancel_url' => array(
                'title'       => __('Cancel Affirm Page', 'woocommerce-gateway-affirm'),
                'type'        => 'select',
                'description' => __('Choose where to send user if payment is cancelled', 'woocommerce-gateway-affirm'),
                ...
                'options'     => array(
                    ...
                    self::CANCEL_TO_CHECKOUT => __('Checkout Page', 'woocommerce-gateway-affirm'),
                    self::CANCEL_TO_CUSTOM => __('Custom URL', 'woocommerce-gateway-affirm')
                ),
            ),
            'custom_cancel_url' => array(
                'title'       => __('Permalink/Custom Cancel URL', 'woocommerce-gateway-affirm'),
                'type'        => 'text',
                'description' => __('Specify where to redirect users when Custom URL is selected for when Affirm payment is cancelled  (Use Permalink or URL in the same domain)', 'woocommerce-gateway-affirm'),
                'default'     => '',
            ),
            'ala_settings' => array(
                'title'       => __('Promotional Messaging Settings', 'woocommerce-gateway-affirm'),
                'type'        => 'select',
                'default'     => 'expand',
                'options'     => array(
                    self::MINIMIZE => __('Minimize', 'woocommerce-gateway-affirm'),
            ...
            'productALA_options' => array(
                'type'        => 'select',
                'description' => __('Choose where the promotional messaging gets rendered on product page', 'woocommerce-gateway-affirm'),
                'default'     => self::AFTER_PRODUCT_PRICE,
                'options'     => array(
                    self::AFTER_PRODUCT_PRICE => __('After product price (Default)', 'woocommerce-gateway-affirm'),
                    self::AFTER_ADD_TO_CART => __('After add to cart', 'woocommerce-gateway-affirm'),
                ),
            ),
            'cartALA' => array(
            ...
            'affirm_color' => array(
                ...
                'type'        => 'select',
                'description' => __('Affirm logo/text color on the promotional payment messaging.', 'woocommerce-gateway-affirm'),
                'default'     => 'blue',
                'options'     => array(
            ...
            'show_learnmore' => array(
                ...
                'type'        => 'checkbox',
                'description' => __('Show Learn More link in promotional payment messaging.', 'woocommerce-gateway-affirm'),
                'default'     => 'yes',
            ),
            'promo_id' => array(
            ...
            'advance_settings' => array(
                'title'       => __('Advance Settings', 'woocommerce-gateway-affirm'),
                'type'        => 'select',
                'default'     => 'minimize',
                'options'     => array(
                    self::MINIMIZE => __('Minimize', 'woocommerce-gateway-affirm'),
            ...
            'show_fee' => array(
                'title'       => __('Display Affirm fee', 'woocommerce-gateway-affirm'),
                'type'        => 'checkbox',
                'label' => __('Display merchant fee', 'woocommerce-gateway-affirm.'),
                'description' => __('Display the portion of the captured amount that represents the mertchant fee. For any refunds initiated outside of Woocommerce, refunded fee will not be reflected in the shown amount.', 'woocommerce-gateway-affirm.'),
                'default'     => 'yes',
                'desc_tip'    => true,
            ),
        );
    ...
    function validateOrderFromRequest()
    {
        ...
        if( WC()->cart->get_cart_hash() !== $_GET['cart_hash'] ) {
            if (!wp_verify_nonce($_GET['nonce'], 'affirm-checkout-order-' . $order_id)) {
                return false;
            }
        }
    ...
    function enqueueScripts()
    {
        ...
        } else if ($this->cancel_url == self::CANCEL_TO_CUSTOM && $this->custom_cancel_url != ''  ) {
            $custom_url = $this->custom_cancel_url;
            $cancel_url = html_entity_decode($order->get_cancel_order_url(html_entity_decode($custom_url)));
        } else {
