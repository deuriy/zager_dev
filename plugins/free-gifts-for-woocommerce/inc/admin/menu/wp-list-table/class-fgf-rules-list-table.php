...
if ( ! class_exists( 'FGF_Rules_List_Table' ) ) {
	...
	class FGF_Rules_List_Table extends WP_List_Table {
		...
		public function render() {
			...
			$this->prepare_items() ;
			$this->views() ;
			$this->search_box( esc_html__( 'Search Rule' , 'free-gifts-for-woocommerce' ) , 'fgf-rules' ) ;
			$this->display() ;
		}
		...
		public function get_views() {
			...
			$status_link_array = apply_filters( $this->list_slug . '_get_views' , array(
				...
				'fgf_inactive' => esc_html__( 'In-Active' , 'free-gifts-for-woocommerce' ) ,
					) ) ;
		...
		private function render_product_category( $item ) {
			...
			if ( '4' == $item->get_rule_type() ) {
				...
				$coupon_products = esc_html__( 'Coupon Code to Apply' , 'free-gifts-for-woocommerce' ) ;

				$coupons_link  = $this->get_coupons_link( $item->get_apply_coupon() ) ;
				$products      = $this->get_products_link( $item->get_coupon_gift_products() ) ;
				$products_link = esc_html__( 'Gift Product(s)' , 'free-gifts-for-woocommerce' ) . ' ' . $products ;
				...
				return '<b><u>' . esc_html__( 'Categories' , 'free-gifts-for-woocommerce' ) . '</u></b><br />' . $categories_link ;
			} else {
				$products_link = $this->get_products_link( $item->get_gift_products() ) ;

				return '<b><u>' . esc_html__( 'Product(s)' , 'free-gifts-for-woocommerce' ) . '</u></b><br />' . $products_link ;
			}
		}
