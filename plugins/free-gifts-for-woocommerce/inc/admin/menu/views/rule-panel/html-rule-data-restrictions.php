			...
			<tr>
				<th scope='row'>
					<label><?php esc_html_e( 'Order Restriction per User' , 'free-gifts-for-woocommerce' ) ; ?><span class="required">*</span>
						<?php fgf_wc_help_tip( esc_html__( 'When set to Enable, registered users can be restricted to receive free gift(s) from this rule for a fixed number of times.' , 'free-gifts-for-woocommerce' ) ) ; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped. ?>
					</label>
				</th>
				<td>
			...
			<tr>
				<th scope='row'>
					<label><?php esc_html_e( 'Number of Order(s) per User' , 'free-gifts-for-woocommerce' ) ; ?>
						<?php fgf_wc_help_tip( esc_html__( 'The number of order(s) for which each registered users can receive gift product(s) from this rule.' , 'free-gifts-for-woocommerce' ) ) ; // phpcs:ignore WordPress.XSS.EscapeOutput.OutputNotEscaped. ?>
					</label>
				</th>
				<td>
					<input type="number" class="fgf_rule_allowed_user_count" name="fgf_rule[fgf_rule_allowed_user_count]" min="0" value="<?php echo esc_attr( $rule_data[ 'fgf_rule_allowed_user_count' ] ) ; ?>"/>
				</td>				
			</tr>
