...
namespace PaymentPlugins\WooFunnels\Braintree\Payments;


use PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\BasePaymentGateway;
...
class Api {
	...
	public function __construct( $plugin_version, $js_sdk_version ) {
	...
	private function get_payment_gateways() {
		return [
			'braintree_cc'        => 'PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\CreditCardGateway',
			'braintree_paypal'    => 'PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\PayPalGateway',
			'braintree_googlepay' => 'PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\GooglePayGateway',
			'braintree_applepay'  => 'PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\ApplePayGateway',
			'braintree_venmo'     => 'PaymentPlugins\WooFunnels\Braintree\Payments\Gateways\VenmoGateway'
		];
	}
	...
	public function enqueue_scripts() {
		...
		if ( $this->is_braintree_payment_method( $payment_method ) ) {
			...
			if ( $payment_gateway->is_vaulted_threed_secure_enabled() ) {
				...
				$params     = require_once dirname( dirname( __DIR__ ) ) . '/build/wc-braintree-woofunnels.asset.php';
				...
				wp_enqueue_script( 'wc-braintree-woofunnels', $assets_url . 'wc-braintree-woofunnels.js', $params['dependencies'], $this->plugin_version, true );
				$wp_scripts->do_items( [ 'wc-braintree-woofunnels' ] );
			}
		}
	}
