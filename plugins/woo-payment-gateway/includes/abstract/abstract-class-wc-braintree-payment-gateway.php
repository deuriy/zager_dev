...
abstract class WC_Braintree_Payment_Gateway extends WC_Payment_Gateway {
	...
	public function should_save_payment_method() {
		...
		return ( ! empty( $_POST[ $this->save_method_key ] ) && ! $this->use_saved_method() ) || ( wcs_braintree_active() && ! $this->use_saved_method() &&
		                                                                                           ( WC_Subscriptions_Cart::cart_contains_subscription() || wcs_cart_contains_renewal() ) && $this->supports( 'subscriptions' ) );
	}
			...
			if ( $response->success ) {
				...
				if ( $errors ) {
					foreach ( $errors->shallowAll() as $error ) {
						if ( $error->code === '91506' ) {
							$message .= ' ' . __( 'If you wish to cancel the transaction you can set the order\'s status to cancelled or use the void option.', 'woo-payment-gateway' );
						}
					}
				}
	...
	protected function add_order_line_items( &$args, $order, &$items = array() ) {
		if ( $this->is_active( 'line_items' ) ) {
			...
			foreach ( $order->get_items( 'line_item' ) as $item ) {
				...
				if ( 0 < round( $item->get_subtotal(), $this->line_item_validations['totalAmount'] ) ) {
					$line_item = array(
						...
						'unitAmount'     => wc_format_decimal( $item->get_subtotal() / $item->get_quantity(), $this->line_item_validations['unitAmount'] ),
						'taxAmount'      => wc_format_decimal( $item->get_subtotal_tax(), $this->line_item_validations['taxAmount'] ),
						'discountAmount' => wc_format_decimal( $item->get_subtotal() - $item->get_total(), $this->line_item_validations['discountAmount'] )
					);
			...
			foreach ( $order->get_fees() as $fee ) {
				 ...
				 */
				if ( 0 < round( $fee->get_total(), $this->line_item_validations['totalAmount'] ) ) {
					$item = array(
						'name'        => sprintf( __( 'Fee - %s', 'woo-payment-gateway' ), $fee->get_name() ),
						'kind'        => 'debit',
						'quantity'    => intval( $fee->get_quantity() ),
						'totalAmount' => wc_format_decimal( $fee->get_total(), $this->line_item_validations['totalAmount'] ),
						'unitAmount'  => wc_format_decimal( $fee->get_total() / $fee->get_quantity(), $this->line_item_validations['unitAmount'] ),
						'taxAmount'   => wc_format_decimal( $fee->get_total_tax(), $this->line_item_validations['taxAmount'] )
					);
			...
			foreach ( $order->get_items( 'coupon' ) as $coupon ) {
				...
				if ( 0 < round( $coupon->get_discount(), $this->line_item_validations['totalAmount'] ) ) {
					$item = array(
						...
						'unitAmount'  => wc_format_decimal( abs( $coupon->get_discount() ) / $coupon->get_quantity(), $this->line_item_validations['unitAmount'] ),
			...
			if ( 0 < ( round( $order->get_discount_total() + $this->order_totals['discount']['subtotal'], $this->line_item_validations['totalAmount'] ) ) ) {
				$item = array(
					'name'        => __( 'Discount', 'woo-payment-gateway' ),
					'description' => sprintf( __( 'Discount for order %s', 'woo-payment-gateway' ), $order->get_id() ),
					'kind'        => 'credit',
					'quantity'    => 1,
					'totalAmount' => wc_format_decimal( $order->get_discount_total() + $this->order_totals['discount']['subtotal'], $this->line_item_validations['totalAmount'] ),
					'unitAmount'  => wc_format_decimal( $order->get_discount_total() + $this->order_totals['discount']['subtotal'], $this->line_item_validations['unitAmount'] ),
					'taxAmount'   => wc_format_decimal( $order->get_discount_tax() + $this->order_totals['discount']['tax_total'], $this->line_item_validations['taxAmount'] )
				);
	...
	public function get_display_items_for_cart( $cart, $decimals ) {
		...
		foreach ( $cart->get_cart() as $key => $cart_item ) {
			$total   = wc_format_decimal( $incl_tax ? wc_get_price_including_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'] ) ) : wc_get_price_excluding_tax( $cart_item['data'], array( 'qty' => $cart_item['quantity'], ) ), $decimals );
		...
		if ( WC()->cart->needs_shipping() && WC()->cart->show_shipping() ) {
			$total   = wc_format_decimal( $incl_tax ? WC()->cart->shipping_total + WC()->cart->shipping_tax_total : WC()->cart->shipping_total, $decimals );
		...
		if ( 0 < WC()->cart->discount_cart ) {
			$total   = wc_format_decimal( - 1 * abs( $incl_tax ? WC()->cart->discount_cart + WC()->cart->discount_cart_tax : WC()->cart->discount_cart ), $decimals );
			...
			foreach ( $recurring_cart->get_cart() as $cart_item ) {
				if ( ( wcs_braintree_active() && WC_Subscriptions_Product::get_trial_length( $cart_item['data'] ) > 0 ) ||
					...
					if ( $recurring_cart->needs_shipping() ) {
						...
						$total   = wc_format_decimal( $incl_tax ? $recurring_cart->shipping_total + $recurring_cart->shipping_tax_total : $recurring_cart->shipping_total, $decimals );
					...
					if ( 0 < $recurring_cart->discount_cart ) {
						...
						$total   = wc_format_decimal( - 1 * ( $incl_tax ? $recurring_cart->discount_cart + $recurring_cart->discount_cart_tax : $recurring_cart->discount_cart ), $decimals );
